...
function classifyAfterAiReview(validation, articles = [], review = null) {
  const base = decisionFromValidation(validation, articles);
  if (!review || validation.stage === 'blocked') {
    return { ...base, classified_by: 'rules' };
  }
  if (review.verdict === 'supported') {
    return {
      ...
      label: '작성 가능',
      reason: review.reason || 'AI 검토 결과 요약이 근거와 맞아 작성 가능입니다.',
      classified_by: 'ai',
    };
  }
  return {
    allowed: false,
    needs_source_read: review.verdict === 'insufficient',
    label: '작성 보류',
    reason: review.reason || (review.verdict === 'insufficient'
      ? 'AI 검토 결과 요약 근거가 부족해 보류입니다.'
      : 'AI 검토 결과 요약 확인이 필요해 보류입니다.'),
    classified_by: 'ai',
  };
}

function applyReviewToDigest(digest, validation, articles, review) {
  if (!digest) return digest;
  digest.decision = classifyAfterAiReview(validation, articles, review);
  return digest;
}
