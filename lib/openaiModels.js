const MODEL_DEFAULTS = Object.freeze({
  classify: 'gpt-5.4-nano',
  suggest: 'gpt-5-mini',
  draft: 'gpt-5-mini',
  premium: 'gpt-5.4-mini',
});

const ALLOWED_MODELS = new Set([
  'gpt-5.4-nano',
  'gpt-5-mini',
  'gpt-5.4-mini',
]);

const IMAGE_MODEL_DEFAULT = 'gpt-image-2';
const ALLOWED_IMAGE_MODELS = new Set(['gpt-image-2']);

function resolveOpenAIModel(task, requestedModel) {
  const envName = `OPENAI_MODEL_${task.toUpperCase()}`;
  const model = requestedModel || process.env[envName] || MODEL_DEFAULTS[task];

  if (!model || !ALLOWED_MODELS.has(model)) {
    throw new Error(
      `지원하지 않는 OpenAI 모델입니다: ${model || '(비어 있음)'}. ` +
        `허용 모델: ${Array.from(ALLOWED_MODELS).join(', ')}`
    );
  }

  return model;
}

function resolveOpenAIImageModel(requestedModel) {
  const model = requestedModel || process.env.OPENAI_IMAGE_MODEL || IMAGE_MODEL_DEFAULT;
  if (!ALLOWED_IMAGE_MODELS.has(model)) {
    throw new Error(`Unsupported OpenAI image model: ${model || '(empty)'}.`);
  }
  return model;
}

module.exports = {
  ALLOWED_IMAGE_MODELS,
  ALLOWED_MODELS,
  IMAGE_MODEL_DEFAULT,
  MODEL_DEFAULTS,
  resolveOpenAIImageModel,
  resolveOpenAIModel,
};
